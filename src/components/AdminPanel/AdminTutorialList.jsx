...
import {
  useDeleteByAdminTutorialMutation,
  useGetAllTutorialsQuery,
} from "../features/lessons/tutorialApi";
import { setSelectedTutorial } from "../features/lessons/tutorialSlice";
...
const AdminTutorialList = () => {
  ...
  const [deleteByAdminTutorial, { isLoading: deleteTutorialLoading }] =
    useDeleteByAdminTutorialMutation();
  ...
  const handleDeleteTutorial = async (tutorialId) => {
    try {
      const confirmation = confirm("Are you sure to delete this Tutorial?");
      if (confirmation) {
        const result = await deleteByAdminTutorial(tutorialId).unwrap();
      ...
      toast.error("Delete Tutorial Have Some Error");
    }
  };

  let content = null;
  if (isLoading || deleteTutorialLoading) {
    content = <Loading />;
  }
  ...
  if (!isLoading && !isError && tutorials?.data.length === 0) {
    content = <div className="text-red-500">There Have no Tutorials!!</div>;
  }
  if (!isLoading && !isError && tutorials?.data.length > 0) {
    content = (
      <table className="min-w-full bg-white">
        ...
        <tbody className="whitespace-nowrap">
          {tutorials.data.map((tutorial, index) => (
            <tr key={tutorial._id} className="even:bg-blue-50">
  ...
  return (
    <main className="">
      <h1 className="py-2 text-2xl font-semibold text-center">Tutorial List</h1>
